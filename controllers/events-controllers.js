...
const getEventById = async (req, res, next) => {
  //the first argument defined here will be added to the end of the path we define in app.js for this route...so in this case /api/places/:pid
...
const createEvent = async (req, res, next) => {
  ...
  if (!errors.isEmpty()) {
    ...
    ); //note: now that we use next() instead of throw (because of asynchronous code) we need to return so that code execution won't continue. With throw returning to stop was not necessary
  }
  ...
  try {
    ...
    const error = new HttpError(
      "Creating event failed, please try again 1",
      500
    );
  ...
  try {
    ...
    const error = new HttpError(
      "Creating event failed, please try again 2",
      500
    );
...
const deleteEvent = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError(
      "Something went wrong, could not delete an event 1",
      500
    );
  ...
  try {
    ...
    const error = new HttpError(
      "Something went wrong, could not delete an event 2",
      500
    );
...
const getCommentsByEventId = async (req, res, next) => {
  ...
  if (!comments) {
    //do not put comments.length===0 here, because otherwise error message will be played in events.js (frontend) even if no comments exist for a place...so this error message should be provided only if there is no response above, though I think the catch block above should handle those cases already

    return next(
      new HttpError("Could not find a comment for the provided id.", 404)
    );
  }
...
const deleteComments = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError(
      "Something went wrong, could not delete an comment 1",
      500
    );
  ...
  if (comments[0].creator.id !== req.userData.userId) {
    //req.userData comes from localStorage...so
  ...
  try {
    ...
    const error = new HttpError(
      "Something went wrong, could not delete events 2",
      500
    );
...
const deleteCommentsUserDeletion = async (req, res, next) => {
  ...
  if (comments[0].creator.toString() !== req.userData.userId) {
    //req.userData comes from localStorage...so the req carries localStorage items in it
...
const deleteEvents = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError(
      "Something went wrong, could not delete events 1",
      500
    );
  ...
  if (events[0].creator.toString() !== userId) {
    //req.userData comes from localStorage...so the req carries localStorage items in it

    const error = new HttpError(
      "You are not allowed to delete these events.",
      404
    );
