...
const signup = async (req, res, next) => {
  ...
  if (!errors.isEmpty()) {
    ...
    ); //can't use throw here, because we are inside an asynchronous task (the whole function is asynchronous block)
  }
  ...
  //NOTE2: for some reason when I updated the app by adding an option to delete user, the above string modification gave the same error as above...this time I fixed it by using req.file.path instead (first time it appeared I needed to use String() to fix the issue).
  //Might be the issue is related to different code editors?? Both req.file.path and String(req.file.path) will return type of string so I'm not sure why this error keeps happening.
  ...
  try {
    token = jwt.sign(
      ...
      process.env.JWT_KEY,
      { expiresIn: "1h" }
    ); //the second argument is so called "private key". Only server side knows this. So even if someone manages to decrypt the password in the front-end side the token wouldn't be right without knowing the private key. See json web token documentation for the third argument (instructions 93) ). process.env.JWT_KEY is from nodemon.json (process is Node.js way of accessing the file and env is the property set there)
  } catch (err) {
    const error = new HttpError("Signing up failed, please try again", 500);
...
const login = async (req, res, next) => {
  ...
  if (!isValidPassword) {
    // we are doing this check to see if the password existed.
...
const deleteUser = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError(
      "Something went wrong, could not delete the user 1",
      500
    );
  ...
  try {
    ...
    const error = new HttpError(
      "Something went wrong, could not delete the user 2",
      500
    );
