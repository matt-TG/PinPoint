...
router.post('/', fileUpload.single('image'), [check('title').not().isEmpty(), check('description').isLength({min:5}), check('address').not().isEmpty(), check('link').not().isEmpty(), check('date').not().isEmpty()], eventsControllers.createEvent); //requests can accept more than one Middleware...reads from left to right. check('title').not().isEmpty() means that we check that title is not empty

router.post('/comment', [check('comment').not().isEmpty()], eventsControllers.createComment);

router.patch('/:eid', [check('title').not().isEmpty(), check('description').isLength({min:5})], eventsControllers.updateEvent); //for updating a place
